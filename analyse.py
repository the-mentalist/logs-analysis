...
"""
View should be created beforehand, if not then uncomment this

# drop view if already exists
cursor.execute(
    'drop view if exists articlecount;'
)
...
cursor.execute(
    "select name,sum(count) from articlecount,authors " +
    "where authors.id=author group by name order by sum desc;"
)
...
# here first query returns total request per day
# and second returns errors per day
# both are joined to get percentage error
cursor.execute(
    "select to_char(total.date,'Mon DD, YYYY'), round(ecount*100.0/tcount,2)" +
    "as percentage_error " +
    "from (select date(time), count(status) as tcount from log " +
    "group by date(time)) as total " +
    "left join (select date(time), count(status) as ecount from log " +
    "where status not like '%OK' " +
    "group by date(time)) as error on total.date=error.date " +
